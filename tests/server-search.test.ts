...
afterEach(() => {
  ...
  clearAccountQuota();
  clearRequestLogsForTests();
  // Account-store ACL workers can release their Windows handles just after server.stop().
  // Bound that filesystem-release race without swallowing a persistent cleanup failure.
  if (existsSync(TEST_DIR)) removeTreeWithRetry(TEST_DIR);
});
