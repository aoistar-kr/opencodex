...
export function buildProviderTableBlockForTarget(
  ...
  if (supportsStandaloneWebSearch) lines.push("supports_standalone_web_search = true");
  return lines.join("\n") + "\n";
}
...
export function buildProfileFileForTarget(
  ...
  fastMode?: boolean,
  supportsStandaloneWebSearch = false,
  standaloneWebSearchEnabled?: boolean,
): string {
  ...
  if (featureLines.length > 0) lines.push("", "[features]", ...featureLines);
  lines.push(buildProviderTableBlockForTarget(target, supportsWebsockets, supportsStandaloneWebSearch).trimEnd(), "");
  return lines.join("\n");
}
