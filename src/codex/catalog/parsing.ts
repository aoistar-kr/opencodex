...
export function normalizeRoutedCatalogEntry(
  ...
  // code-mode row advertises deferred discovery. Cursor omits hosted web-search metadata because its
  // runTurn path bypasses that sidecar; OpenCode Go omits it because search is owned exclusively by
  // Codex standalone `web.run`. Disabling supports_search_tool is a separate payload regression.
