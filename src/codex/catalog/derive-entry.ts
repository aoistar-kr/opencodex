...
export function deriveEntry(
  ...
  if (template || codexForwardNativeCapabilityAlias) {
    ...
    if (isRouted) {
      ...
      if (model?.provider === "opencode-go") delete e.web_search_tool_type;
  ...
  const isOpenCodeGoFallback = isRouted && model?.provider === "opencode-go";
  const entry: RawEntry = {
    ...
    ...(isRouted
      ? isCursorFallback || isOpenCodeGoFallback
  ...
  if (model?.provider === "opencode-go") delete entry.web_search_tool_type;
