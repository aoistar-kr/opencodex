...
async function handleChatCompletionsWithBudget(
  ...
  if (!logCtx.conversationId) {
    const chatConversationIdentity = req.headers.get("x-codex-parent-thread-id")
      ?? sessionIdHeaderFromRequest(req.headers)
      ?? req.headers.get("thread-id")
      ?? req.headers.get(OPENCODE_GO_SESSION_HEADER)
      ?? (typeof chatBody.prompt_cache_key === "string" ? chatBody.prompt_cache_key : undefined);
    logCtx.conversationId = normalizeLogConversationId(chatConversationIdentity);
  }
  ...
  try {
    ...
    route.provider = withOpenCodeGoSession(route.provider, logCtx.conversationId);
    ...
    if (isOpenCodeGoTransport(route.provider) && !logCtx.conversationId) {
      if (logIds) addFinalRequestLog(logIds.requestId, logIds.start, logCtx, 400, { closeReason: "non_stream" });
      return chatCompletionsErrorResponse(400, OPENCODE_GO_SESSION_REQUIRED_MESSAGE, "invalid_request_error");
    }
    settledRoute = route;
  ...
  // A JSON Responses body is not a client-visible success until the Chat projection succeeds.
  // Keep deferred request logging for real SSE only; JSON is finalized after translation below.
  ...
  const serialized = stream
    ? jsonCompletionSse(completion, requestedModel, translatorBudget)
    : JSON.stringify(completion);
  if (!stream) {
    translatorBudget.chargeRetained(Buffer.byteLength(serialized) * 2, { kind: "live_transient" });
  }
  return finishJson(new Response(serialized, {
    status: 200,
    headers: stream
      ? {
        "Content-Type": "text/event-stream; charset=utf-8",
