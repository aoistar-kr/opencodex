...
import { CODEX_STANDALONE_WEB_RUN_WIRE_NAME } from "../../codex/standalone-web-search-authority";
...
export async function preparePassthroughExchange(
  ...
  const {
    ...
    const refreshUndeclaredToolGuard = (builtRequest: AdapterRequest): void => {
      ...
      // `web.run` is a Codex-owned out-of-band client tool, so it is absent from tools[]. The
      // request-scoped authority bit was proven after final routing in request-prepare and must
      // survive previous_response_id replay without admitting any other historical tool name.
      if (parsed._standaloneWebRunAuthorized === true) {
        declaredWireToolNames.add(CODEX_STANDALONE_WEB_RUN_WIRE_NAME);
      }
