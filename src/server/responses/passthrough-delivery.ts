...
import { compactionOriginFingerprint } from "../../responses/compaction";
...
export async function deliverPassthroughResponse(
  ...
  const { requestBindings } = transportState;
   ...
   if (usesCodexForwardPoolAuth(admissionState.authCtx, route.provider)) {
    ...
    // A canonical native compaction turn returns an opaque backend-owned compaction item. Buffer
    // only this internal compaction response through its protocol terminal so we can attach the
    // portable shadow before Codex persists it. Ordinary turns keep the low-copy relay below.
    if (
      ...
      await new Promise<void>(resolve => {
        consumeForInspection(
          ...
          { upstream, onParsedPayload: noteInspectedPayload },
        );
      });
      ...
      const portable = await addPortableShadowToSnapshot({
        ...
        transport: {
          ...
          headers: new Headers(nativeExchange.request.headers),
          model: route.modelId,
          connectMs,
          url: nativeExchange.request.url,
        },
      });
      ...
      const compactionHeaders = new Headers(headers);
