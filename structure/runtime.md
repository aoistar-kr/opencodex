...
The Responses adapter removes ChatGPT-auth-only top-level `access_programs` before any noncanonical
Responses dispatch while preserving it on the canonical ChatGPT forward route; the detailed boundary
lives in [Responses transport](transports/responses.md#responses-httpsse).
