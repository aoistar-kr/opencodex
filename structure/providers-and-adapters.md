...
| `src/adapters/openai-responses.ts` | Native OpenAI/ChatGPT Responses passthrough. An explicitly trusted Codex-aware loopback preserves Responses item IDs even with `store: false`: current Codex clears private per-item metadata before a non-OpenAI base URL, while that local runtime uses the IDs as lineage evidence and never resolves them against an upstream response store. Ordinary routed destinations keep stripping unstored IDs. |
